...
\chapterimage{Example4Pic.jpg} % Chapter heading image
...
\noindent {\large \textbf{Example 1: Indeterminate Quotients}}
\noindent {\large\[ \lim_{x \to \infty} \frac{x}{\ln{(1+2e^x)}} \]}
...
\noindent {\large \textbf{Example 2: Indeterminate Products}}
...
\noindent Like always, we try to substitute the x-value we are approaching into the equation first. This time, it actually gives us an indeterminate form of $(0 \cdot -\infty)$.
In order to manipulate this into an indeterminate quotient, we'll need to bring one of the terms into the denominator. We need to make sure we are not changing the overall expression.
Note that the following step is equivalent to the one before (negative power in denom.) We can then apply L'Hopital's rule; we have an indeterminate form of $\frac{0}{0}$.
{\large\[ = \lim_{x \to 0^+} \frac{\ln{x}}{x^\frac{-1}{2}} \]
\[ \stackrel{\text{L'H}}{=} \lim_{x \to 0^+} \frac{\frac{1}{x}}{\frac{-1}{2}x^\frac{-3}{2}} = -2 \lim_{x \to 0^+} \frac{x^\frac{3}{2}}{x} = \lim_{x \to 0^+} -2 \sqrt{x} = -2 \sqrt{0} = 0\]

\noindent {\large \textbf{Example 3: Indeterminate Differences}}

\noindent {\small The indeterminate form $\pm \infty \pm \infty$ is an indeterminate difference form. With these forms, we will try to manipulate it to one of the previous forms to solve it.}
{\large\[ \lim_{x \to 1} \frac{1}{\ln{x}} - \frac{1}{x-1} \]}

\noindent {\small Plugging in $x=1$ will give us ($\infty - \infty$). Since we have two fractions, we want to manipulate it so it is one (Indeterminate Quotient).
    Our first step will be to get common denominators, then apply L'Hopital's rule wherever applicable. \textbf{NOTE:} always try to simplify after applying L'Hopital's rule to make it cleaner.}

{\large\[ = \lim_{x \to 1} \frac{x-1- \ln{x}}{(\ln{x})(x-1)} \]}
{\large\[ \stackrel{\text{L'H}}{=} \lim_{x \to 1} \frac{1 - \frac{1}{x}}{\frac{1}{x}(x-1)+\ln{x}} \cdot \frac{x}{x} \]}
{\large \[ = \lim_{x \to 1} \frac{x-1}{x-1+x\ln{x}} \]}
{\large\[ \stackrel{\text{L'H}}{=} \lim_{x \to 1} \frac{1}{1+1 \cdot \ln{x} + x\frac{1}{x}} \]}
{\large \[ = \frac{1}{1+0+1 \cdot \frac{1}{1}} = \frac{1}{2}\]}

\noindent {\large \textbf{Example 4: Indeterminate Powers}}

\noindent {\small $\infty^0$, $0^0$, and $1^\infty$ are all examples of an indeterminate power. With the case of indeterminate powers, we will be carefully
manipulating the expression with $e^{\ln{x}}$. Note that this \textbf{cancels out to x / is equal to x}.}
{\large\[ \lim_{x \to \infty} x^\frac{1}{x} \]}
{\large \[ =\lim_{x \to \infty} (e^{\ln{x}})^{\frac{1}{x}}\]}
{\large \[ =\lim_{x \to \infty} e^{\frac{\ln{x}}{x}}\]}
\noindent {\small Since $e^x$ is continuous, we can bring the limit in.}
{\large \[ =e^{\lim_{x \to \infty} \frac{\ln{x}}{x}}\]}
\noindent {\small Now we apply L'Hopital's rule since it is applicable.}
{\large \[ \stackrel{\text{L'H}}{=} e^{\lim_{x \to \infty} \frac{(\frac{1}{x})}{1}} \]}
\noindent {\small The limit in the power is 0: $ \lim_{x \to \infty} (x^{\frac{1}{x}}) = 1$}
{\large \[=e^0 = 1\]}

\section{Approximations (AP)}\index{Approximations}
\noindent {\large \textbf{Linearization}} \\

\includegraphics[scale=0.8]{Approx.jpg} \\
\noindent {\small \textbf{Linear approximation} is a process that is used to find a good estimate about the value of a function we \textbf{don't know},
    by using information about a value of a function we \textbf{already know}. Basically, you find the tangent line of a point you \textbf{do know} that is
    close to the point you are trying to estimate - by using the point-slope formula and finding the slope/derivative at that point. Then, you plug in the x-value
    you are trying to find in the tangent line and solve for its y-value. Since the tangent line should be relatively close to the point you are trying to estimate,
    the y-value should be relatively close as well.}

\vspace*{4mm}

\noindent {\large \textbf{Differentials}}\\

\includegraphics{Differentials.png} \\
\noindent {\small For differentials, we utilize the fact that $f'(x)=\frac{dy}{dx}$. We isolate for $dy$ and get $dy=f'(x)dx$. This is saying that the change in
    y is equivalent to the slope/derivative at x, times the change in x.}

\includegraphics[scale=0.7]{Differentials2.jpg}

\noindent {\small This is helpful as when trying to estimate a number such as $\sqrt{16.4}$, we can 'split' it up into $\sqrt{16} +\sqrt{0.4}$. Then we can say that the change in y
    (from $\sqrt{16}$ to $\sqrt{16.4}$) is the derivative at 16 $\cdot$ the change in x - which is 0.4, and add $\Delta y$ to $\sqrt{16}$. Note this is an \textbf{approximation}.}
\end{document}
